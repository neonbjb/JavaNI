	...
	m_nHistorySize(nHistory), m_DepthGenerator(depthGenerator), m_bDrawDM(false), m_bFrameID(false)
{
	...
	int err = getaddrinfo(NULL, TRACE_PORT, &hints, &result);
	if(err != 0){
		printf("getaddrinfo failed: %d\n", err);
		return;
	}
	traceServerSocket = socket(result->ai_family, result->ai_socktype, result->ai_protocol);
	if(traceServerSocket == INVALID_SOCKET){
		printf("Error at socket(): %1d\n", WSAGetLastError());
		WSACleanup();
		return;
	}
	printf("Waiting for a client to connect on port %s\n", TRACE_PORT);
	err = bind(traceServerSocket, result->ai_addr, (int)result->ai_addrlen);
	if(err == SOCKET_ERROR){
	...
	freeaddrinfo(result);
	if(listen(traceServerSocket, SOMAXCONN) == SOCKET_ERROR){
		printf("Listen failed with error: %1d\n", WSAGetLastError());
		closesocket(traceServerSocket);
		return;
	}
	traceSocket = INVALID_SOCKET;
	traceSocket = accept(traceServerSocket, NULL, NULL);
	printf("Client connected, continuing hand server\n");
}
...
void XnVPointDrawer::OnPointUpdate(const XnVHandPointContext* cxt)
{
	...
	//send out trace data to the client
	if(traceSocket != INVALID_SOCKET){
		sprintf(strBuf, "%f,%f,%f\n", ptProjective.X, ptProjective.Y, ptProjective.Z);
		int res = send(traceSocket, strBuf, strlen(strBuf)+1, 0);
		if(res == SOCKET_ERROR){
			printf("Socket error, invalidating socket..\n");
			traceSocket = INVALID_SOCKET;
		}
	}
